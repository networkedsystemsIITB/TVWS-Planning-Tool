...
for fin1 in sorted(glob.glob('./csv/input_places_propagation_model_for_sec_base_stations/*.csv'), key=alphabeticalSort):
    infile = open(fin1,'r')
    ...
    population_file = open('./csv/input_places_population_within_radius/input_places_population_within_radius_'+name, 'r')
    per_row_file = []
    ...
    fid = open('./csv/input_places_propagation_model_for_sec_base_stations_wu_wat/input_places_propagation_model_for_sec_base_stations_wu_wat_'+name, 'w')
    #fid.write('PLACE, POPULATION, LATITUDE, LONGITUDE, ALTITUDE, BANDWIDTH_REQUIRED_PER_USER(Mbps), COVERAGE_RADIUS(kms), WEIGHTED_UTILITY, WEIGHTED_AVERAGE_THROUGHPUT(Mbps)\n')

    for i in range(len(data)):
        fid.write('%s, %d, %f, %f, %.2f, %.2f, %.2f, %.2f, %.2f\n' % (data[i][0], int(data[i][1]), float(data[i][2]), float(data[i][3]), float(data[i][4]), float(data[i][5]), float(data[i][6]), float(data[i][7]), float(data[i][8])))
