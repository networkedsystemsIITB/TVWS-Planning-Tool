...
for fin1 in sorted(glob.glob('./csv/input_places_propagation_model_for_sec_base_stations/*.csv'), key=numericalSort1):
    num = get_number_from_filename(fin1)
    infile = open(fin1,'r')
    per_row = []
    for line in infile:
    ...
    transmit_power_data = per_column[6]     # List of transmit power data values
    coverage_radius_data = per_column[8]    # List of coverage radius data values
    ...
    population_file = open('./csv/input_places_population_within_radius/input_places_population_within_radius_'+str(num)+'.csv', 'r')
    per_row_file = []
    ...
    channel_bandwidth = float(sys.argv[2]) * 10**6  # Channel Bandwidth in Hz, default = 8 MHz
    ...
    for i in range(len(data)):
        ...
        for j in range(len(radius_edge_data)):
            ...
            print "Internet Population Within %d km Radius[%d][%d] = %d" % (radius_edge_data[j], i, j, population_within_radius_data[j])
            ...
            print "Transmit Power[%d] = %f dBm" % (i, float(data[i][6]))
            inter_received_power_dBm = float(data[i][6]) - inter_path_loss[i][j]
    ...
    fid = open('./csv/input_places_propagation_model_for_sec_base_stations_wu_wat/input_places_propagation_model_for_sec_base_stations_wu_wat_'+str(num)+'.csv', 'w')
    #fid.write('PLACE, POPULATION, LATITUDE, LONGITUDE, ALTITUDE, ANTENNA_HEIGHT, TRANSMIT_POWER, BANDWIDTH_REQUIRED_PER_USER(Mbps), COVERAGE_RADIUS(kms), WEIGHTED_UTILITY, WEIGHTED_AVERAGE_THROUGHPUT(Mbps)\n')

    for i in range(len(data)):
        fid.write('%d, %d, %f, %f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n' % (int(data[i][0]), int(data[i][1]), float(data[i][2]), float(data[i][3]), float(data[i][4]), float(data[i][5]), float(data[i][6]), float(data[i][7]), float(data[i][8]), float(data[i][9]), float(data[i][10])))
