...
<li><a href="../Thane/client.html">Thane Model</a></li>
...
if (isset($_POST["submit"]))
{
    if (isset($_FILES["upload"]))
    {
        ...
        {
            $storagename = "London_Original.csv";
...
{
    shell_exec("find ./csv -not -name 'London_Original.csv' -name '*.csv' -type f -delete");
    shell_exec("find ./result -name '*.json' -type f -delete");
    shell_exec("rm -rf /var/lib/mysql-files/*.csv");
    shell_exec("cp csv/London_Original.csv csv/place_pop_lat_long_alt_bw.csv");

    shell_exec("python calculate_coverage_radius_london.py $_POST[propagation_model] $_POST[rx_sensitivity] $_POST[frequency]");
    ...
    $sql = "create table temp_coverage_table1 SELECT Sec_Tx, Sec_Tx_Lat, Sec_Tx_Long, Sec_Tx_Alt, Coverage_Radius_kms, Input_Place AS Sec_Rx, Input_Population AS Sec_Rx_Population, Input_BW_Reqt_Per_User_Mbps AS BW_Reqt_Sec_Rx, round(
        ...
        AS Distance
        FROM `Test_London`.`coverage_data`, `Test_London`.`input_data`
        WHERE (
                6371 * acos (
                cos ( radians(Sec_Tx_Lat) )
                * cos( radians(Input_Latitude) )
    ...
    shell_exec("python propagation_model_input_places_london.py $_POST[propagation_model] $_POST[width] $_POST[frequency]");
    ...
    shell_exec("sort -t, -nrk11,11 -nrk9,9 ./csv/place_pop_lat_long_alt_bw_range_wu_wat.csv -o ./csv/place_pop_lat_long_alt_bw_range_wu_wat.csv");
    ...
    {
        $sql = "create table temp_table1 SELECT Place AS Sec_Tx, Population AS Sec_Tx_Population, Latitude AS Sec_Tx_Lat, Longitude AS Sec_Tx_Long, Altitude AS Sec_Tx_Alt, Antenna_Height AS Sec_Tx_Antenna_Height, Transmit_Power AS Sec_Tx_Transmit_Power, Coverage_Radius_kms, Input_Place AS Sec_Rx, Input_Population AS Sec_Rx_Population, Input_BW_Reqt_Per_User_Mbps AS BW_Reqt_Sec_Rx, round(
                    6371 * acos (
            ...
            FROM `Test_London`.`uval_data`, `Test_London`.`input_data`
            WHERE (
                    6371 * acos (
                    cos ( radians(Latitude) )
                    * cos( radians(Input_Latitude) )
    ...
    $sql = "select Sec_Tx, Sec_Tx_Population, Sec_Tx_Lat, Sec_Tx_Long, Sec_Tx_Alt, Sec_Tx_Antenna_Height, Sec_Tx_Transmit_Power, Coverage_Radius_kms, Sec_Rx, Sec_Rx_Population, BW_Reqt_Sec_Rx, Distance, UVal_Sec_Tx from temp_table1 INTO OUTFILE '/var/lib/mysql-files/select_sec_tx.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n'";
    $conn->query($sql);

    shell_exec("cp /var/lib/mysql-files/select_sec_tx.csv ./csv/select_sec_tx.csv");

    shell_exec("python select_sec_tx_london.py");
    ...
    $sql = "select Sec_Tx, sum(Sec_Rx_Population) AS Aggregate_Population, Sec_Tx_Lat, Sec_Tx_Long, Sec_Tx_Alt, Sec_Tx_Antenna_Height, Sec_Tx_Transmit_Power, max(BW_Reqt_Sec_Rx) AS BW_Reqt, Coverage_Radius_kms from temp_table2 group by Sec_Tx, Sec_Tx_Lat, Sec_Tx_Long, Sec_Tx_Alt, Sec_Tx_Antenna_Height, Sec_Tx_Transmit_Power, Coverage_Radius_kms INTO OUTFILE '/var/lib/mysql-files/propagation_model_for_sec_base_stations_input.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n'";
    $conn->query($sql);
    ...
    shell_exec("python propagation_model_sec_tx_london.py $_POST[propagation_model] $_POST[width] $_POST[frequency]");

    shell_exec("python parse_json_london.py");
