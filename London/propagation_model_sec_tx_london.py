...
for fin1 in sorted(glob.glob('./csv/sec_tx_propagation_model_for_sec_base_stations/*.csv'), key=numericalSort):
    ...
    population_file = open('./csv/sec_tx_population_within_radius/sec_tx_population_within_radius_'+str(num)+'.csv', 'r')
    per_row_file = []
    ...
    header_list = ["place", "population", "lat", "long", "alt", "antenna_height", "tx_power", "bw_reqt", "coverage", "throughput_list", "users_served", "weighted_utility", "weighted_average_throughput"]
    keys = header_list
    ...
    fid = open('./result/result_'+str(num)+'.json', 'w')
